...
def fetch_alerts() -> List[Dict[str, Any]]:
    with get_connection() as connection:
        rows = connection.execute(
            "SELECT id, alert_type, risk_level, message, risk_score, timestamp FROM alerts ORDER BY id DESC"
        ).fetchall()
...
def fetch_latest_risk_score() -> int:
    with get_connection() as connection:
        row = connection.execute("SELECT MAX(risk_score) AS max_score FROM alerts").fetchone()
